...
resource "aws_instance" "web-server-instance" {
  ...
  provisioner "file" {
    source      = "C:\\moaad\\learn\\forsa\\terraform\\moaad-terraform\\flask" #the file we need to copy
    destination = "/home/ubuntu" # where we want him to be

    # make connection to the Ubuntu server
    connection {
    type     = "ssh"
    private_key = "${file("C:\\moaad\\learn\\forsa\\terraform\\moaad-terraform\\moaad-key.pem")}" # RSA private key
    user     = "ubuntu"
  ...
  provisioner "remote-exec" {
    ...
    inline = [ 
     ...
     "sudo curl -L \"https://github.com/docker/compose/releases/download/1.27.4/docker-compose-$(uname -s)-$(uname -m)\" -o /usr/local/bin/docker-compose",
      ...
      "git clone https://github.com/moaaaad//finalproject.git",
      "cd finalproject",
